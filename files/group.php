...
for ($i = 1; $i <= $_SESSION['TOTALPAGES']; $i++) {
    if (isset($_POST['mypage_' . $i])) {
        ...
        update_option('mypage', $_POST['mypage_' . $i]);
        $_SESSION['current'] = get_option('mypage');
...
if (get_option('presource')!=get_option('source') || get_option('pregroup')!=get_option('group')) {
    $_SESSION['current'] = 1;
    update_option('mypage', $_SESSION['current']);
    update_option('presource',get_option('source'));
    update_option('pregroup',get_option('group'));
}
$photourl = 'http://api.flickr.com/services/rest/?method=flickr.groups.pools.getPhotos&api_key=' . get_option('api') . '&group_id=' . get_option('group') . '&per_page=' . $mypage . '&page=' . get_option('mypage') . '&format=php_serial';

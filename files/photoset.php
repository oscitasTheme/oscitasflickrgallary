...
if (get_option('presource')!=get_option('source') || get_option('prephotoset')!=get_option('photoset')) {
    $_SESSION['current'] = 1;
    update_option('mypage', $_SESSION['current']);
    update_option('presource',get_option('source'));
    update_option('prephotoset',get_option('photoset'));
}
$photourl = 'http://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=' . get_option('api') . '&photoset_id=' . get_option('photoset') . '&per_page=' . $mypage . '&page=' . get_option('mypage') . '&format=php_serial';

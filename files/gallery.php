...
if (get_option('presource') != get_option('source') || get_option('pregallery') != get_option('gallery')) {
    $_SESSION['current'] = 1;
    update_option('mypage', $_SESSION['current']);
    update_option('presource', get_option('source'));
    update_option('pregallery', get_option('gallery'));
}
$photourl = 'http://api.flickr.com/services/rest/?method=flickr.galleries.getPhotos&api_key=' . get_option('api') . '&gallery_id=' . get_option('gallery') . '&per_page=' . $mypage . '&page=' . get_option('mypage') . '&format=php_serial';
